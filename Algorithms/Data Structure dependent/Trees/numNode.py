...
def helper(root, s):
  ...
  for child in root.children:
    childNum = helper(child, s)
    for i in range(NUM_LETTER):
      counter[root.val][i] += counter[childNum][i]
  letter = s[root.val - 1]
  counter[root.val][ord(letter) - ord('a')] += 1
  return root.val


def count_of_nodes(root, queries, s):
  global counter
  counter = [[0] * NUM_LETTER  for _ in range(len(s)+1)]
  helper(root, s)
  return [counter[node][ord(letter) - ord('a')] for node, letter in queries]
